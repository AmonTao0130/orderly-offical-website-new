...
async function sendTelegramMessage(
  ...
  if (!botToken || !chatId) {
    return {
      success: false,
      error:
        "Telegram configuration missing. Please check TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID environment variables.",
    };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        error: `Failed to send message to Telegram: ${(errorData as { description?: string }).description || response.statusText}`,
      };
    }
    ...
    return {
      success: false,
      error:
        "Network error while sending message to Telegram. Please try again later.",
    };
  }
}
    ...
    return jsonResponse(
      {
        success: true,
        message:
          "Partnership inquiry submitted successfully. We will get back to you soon!",
      },
      200
    );
  } catch (error) {
