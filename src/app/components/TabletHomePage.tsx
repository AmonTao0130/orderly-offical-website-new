...
function FeatureChip({ dot, label }: { dot: string; label: string }) {
  return (
    <div
      className="flex items-center gap-[10px] px-[20px] py-[14px] rounded-[124px] shrink-0 relative"
      style={{
        background:
    ...
    >
      {/* Gradient border */}
      <div
        className="absolute inset-0 rounded-[124px] pointer-events-none"
        style={{
          padding: '2px',
      ...
      />
      <p className="font-['Atyp_BL:Semibold',sans-serif] text-[12px] text-white leading-none whitespace-nowrap capitalize relative z-10">
        {label}
      </p>
    </div>
  );
...
// Convert FOOTER_NAV to the format expected by the footer
...
// Generate FOOTER_SOCIAL from SOCIAL_LINKS
const FOOTER_SOCIAL = SOCIAL_LINKS.map((href) => ({
  href,
  label: href.includes("discord")
    ? "Discord"
    : href.includes("t.me")
      ? "Telegram"
      : href.includes("x.com")
