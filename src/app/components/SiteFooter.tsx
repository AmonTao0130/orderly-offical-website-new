 ...
 *
 * Exports:
 *   SiteFooter    — desktop footer (≥ 1024px)
 *   TabletFooter  — tablet footer (600–1023px)
 *   MobileFooter  — mobile footer (< 600px), with slide-up nav sheets
 *   BuyOrderModal — Buy ORDER exchange picker modal (reusable standalone)
 *
...
import { FOOTER_NAV, SOCIAL_LINKS, type FooterNavLink } from "@/app/shared/orderly";
...
// ─── Shared social icon row (mobile + tablet) ─────────────────────────────────
// Simpler than the desktop SocialIcons — no absolute positioning inside cells.
function FooterSocialRow({ gap = 16 }: { gap?: number }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap }}>
      <a href={SOCIAL_LINKS[0]} target="_blank" rel="noopener noreferrer" className="hover:opacity-70 transition-opacity flex items-center justify-center" aria-label="Discord">
      ...
      <a href={SOCIAL_LINKS[1]} target="_blank" rel="noopener noreferrer" className="hover:opacity-70 transition-opacity flex items-center justify-center" aria-label="Telegram">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="white">
          <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
        </svg>
      </a>
      <a href={SOCIAL_LINKS[2]} target="_blank" rel="noopener noreferrer" className="hover:opacity-70 transition-opacity flex items-center justify-center" aria-label="YouTube">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="white">
          <path d="M23.495 6.205a3.007 3.007 0 0 0-2.088-2.088c-1.87-.501-9.396-.501-9.396-.501s-7.507-.01-9.396.501A3.007 3.007 0 0 0 .527 6.205a31.247 31.247 0 0 0-.522 5.805 31.247 31.247 0 0 0 .522 5.783 3.007 3.007 0 0 0 2.088 2.088c1.868.502 9.396.502 9.396.502s7.506 0 9.396-.502a3.007 3.007 0 0 0 2.088-2.088 31.247 31.247 0 0 0 .5-5.783 31.247 31.247 0 0 0-.5-5.805zM9.609 15.601V8.408l6.264 3.602z" />
        </svg>
      </a>
      <a href={SOCIAL_LINKS[3]} target="_blank" rel="noopener noreferrer" className="hover:opacity-70 transition-opacity flex items-center justify-center" aria-label="LinkedIn">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="white">
          <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
        </svg>
      </a>
      <a href={SOCIAL_LINKS[4]} target="_blank" rel="noopener noreferrer" className="hover:opacity-70 transition-opacity flex items-center justify-center" aria-label="X (Twitter)">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="white">
...
function FooterCopyrightRow({ px = 28 }: { px?: number }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: `16px ${px}px`, flexWrap: "wrap", gap: 8 }}>
      <p style={style()}>© 2026 Orderly Network</p>
      <div style={{ display: "flex", gap: 20 }}>
        <a href="https://orderly.network/docs/introduction/terms-of-service" target="_blank" rel="noopener noreferrer" style={style({ opacity: 0.8 })}>Terms of Service</a>
        <a href="https://orderly.network/docs/introduction/privacy-policy"   target="_blank" rel="noopener noreferrer" style={style({ opacity: 0.8 })}>Privacy Policy</a>
      </div>
    </div>
  );
}
      ...
      <div style={{ background: "#6700ce", borderRadius: 24, overflow: "hidden", margin: "0 32px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "28px", padding: 28 }}>
          {NAV_SECTIONS.map((section) => (
            <div key={section.heading} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <p style={fontHeading}>{section.heading}</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {section.links.map((link) => (
                  link.href.startsWith("http") || link.target === "_blank" ? (
                    <a key={link.label} href={link.href} target="_blank" rel="noopener noreferrer" style={fontLink} className="hover:opacity-80 transition-opacity">
                      {link.label}
                    </a>
                  ) : (
                    <Link key={link.label} href={link.href} style={fontLink} className="hover:opacity-80 transition-opacity">
                      {link.label}
                    </Link>
                  )
                ))}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 28px", flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(156,117,255,0.2)", borderRadius: 63, padding: "8px 12px" }}>
            ...
            <p style={{ fontFamily: "'atyp-bl-variable','atyp-bl',sans-serif", fontVariationSettings: "'wght' 600", fontFeatureSettings: "'ss03','ss02','ss05','ss06'", fontSize: 12, color: "white", letterSpacing: "0.12px", margin: 0 }}>
              All systems operational.
            </p>
          </div>
          <FooterSocialRow />
...
// ─── MobileFooter ─────────────────────────────────────────────────────────────
/**
 * Slide-up sheet for a single footer nav section (mobile-only).
 * Rendered via portal so it overlays correctly.
 */
    ...
    >
      ...
      >
        <div className="flex items-center justify-between">
          <p style={{ color: "white", fontSize: 16, fontFamily: "'atyp-bl-variable','atyp-bl',sans-serif", fontVariationSettings: "'wght' 700", fontFeatureSettings: "'ss02','ss03','ss05','ss06'", letterSpacing: "0.16px", textTransform: "capitalize", margin: 0 }}>
            {sectionKey}
          </p>
          ...
          {items.map((item) =>
            item.external === true || item.target === "_blank" ? (
              <a key={item.label} href={item.href} target="_blank" rel="noopener noreferrer" onClick={onClose}
                style={{ color: "rgba(255,255,255,0.8)", fontFamily: "'atyp-bl-variable','atyp-bl',sans-serif", fontVariationSettings: "'opsz' 72, 'wght' 500", fontFeatureSettings: "'ss02','ss03','ss05','ss06'", fontSize: 15, textDecoration: "none" }}
...
/**
 * Mobile footer (< 600px).
 * Purple card with: ORDER pill → tappable nav section rows → status + social icons.
 * Tapping a nav row opens a slide-up FooterSheet overlay.
 */
        ...
        <div style={{ background: "#6700ce", borderRadius: 24, overflow: "hidden", margin: "0 16px" }}>
          ...
          <div style={{ padding: "4px 20px" }}>
            {NAV_SECTIONS.map((section, i) => (
              ...
              >
                <span style={{ color: "white", fontSize: 15, fontFamily: "'atyp-bl-variable','atyp-bl',sans-serif", fontVariationSettings: "'wght' 600", fontFeatureSettings: "'ss02','ss03','ss05','ss06'", letterSpacing: "0.15px" }}>
                  {section}
                </span>
