import React, { type ButtonHTMLAttributes, type HTMLAttributeAnchorTarget, type PropsWithChildren } from "react";
...
const buttonVariants = cva(
  [
    "inline-flex justify-center items-center text-white font-medium",
    "text-sm lg:text-base px-[20px] lg:px-[24px] h-[40px] lg:h-[52px] rounded-full cursor-pointer",
  ...
  ],
  {
    variants: {
      type: {
        ...
        outlined: ["border-[1px]", "border-solid", "border-white/[0.8]", "hover:bg-white", "hover:text-black"],
      },
      // disabled: {
...
export interface ButtonProps
  extends Pick<ButtonHTMLAttributes<HTMLButtonElement>, "className" | "onClick" | "disabled">,
  ...
  return (
    <a href={href} target={target}>
      <button
        className={cn(
          buttonVariants({ type }),
          disabled && "text-white/[0.5] hover:text-white/[0.5] cursor-not-allowed",
          type === "secondary" && disabled && "border-white/[0.4] hover:bg-white/[0.08] ",
          type === "outlined" && disabled && "border-white/[0.2]  hover:bg-transparent",
          loading &&
