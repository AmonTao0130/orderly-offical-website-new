 ...
 *
 * Exports:
 *   Desktop:  MorphingHeader, HeaderLogo, CampaignIcon
 *   Mobile:   MobileFixedNav, MobileNavDrawer
 *   Tablet:   TabletNav  (also reuses MobileNavDrawer)
 *
              ...
              <div className="flex flex-row items-center size-full">
                <div className="content-stretch flex gap-[8px] items-center p-[16px] relative size-full flex-1">
                  {item.titleVariant === "perps" ? (
                    <div className="basis-0 content-stretch flex gap-[16px] grow items-center min-h-px min-w-px relative shrink-0">
                      <div className="basis-0 content-stretch flex flex-col gap-[4px] grow items-start justify-center min-h-px min-w-px not-italic relative shrink-0">
                        <p className="font-['Atyp_BL:Regular',sans-serif] font-normal leading-[1.6] relative shrink-0 text-[14px] text-[rgba(255,255,255,0.98)] w-full flex items-center gap-1" style={{ letterSpacing: "0.042em" }}>
                          I{" "}
                    ...
                    <div className="basis-0 content-stretch flex gap-[16px] grow items-center min-h-px min-w-px relative shrink-0">
                      <p className="font-['Atyp_BL:Regular',sans-serif] font-normal leading-[1.6] h-full not-italic relative shrink-0 text-[14px] text-[rgba(255,255,255,0.5)] w-[256px]" style={{ letterSpacing: "0.042em" }}>
                  ...
                  {item.status === "Ongoing" ? (
                    ...
                    >
                      <p className="font-['Atyp_BL:Regular',sans-serif] font-normal leading-[1.6] not-italic relative shrink-0 text-[10px] text-[rgba(255,255,255,0.98)] text-nowrap">Ongoing</p>
                    </div>
                  ) : (
                    <div className="absolute bg-[rgba(255,255,255,0.36)] content-stretch flex items-center justify-center px-[8px] py-[4px] right-0 rounded-bl-[8px] rounded-tr-[8px] top-0">
                      <p className="font-['Atyp_BL:Regular',sans-serif] font-normal leading-[1.6] not-italic relative shrink-0 text-[10px] text-[rgba(255,255,255,0.98)] text-nowrap">Ended</p>
                    </div>
                  )}
                </div>
              </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="font-['Atyp_Text:Medium',sans-serif] text-[16px] leading-none not-italic text-white relative z-10" style={{ letterSpacing: "0.042em" }}>
          Campaigns
        </span>
    ...
    >
      <div className="content-stretch flex flex-col items-start justify-center relative shrink-0">
        <div className="content-stretch flex gap-[2px] items-center relative shrink-0">
          <p className="font-['Atyp_Text:Medium',sans-serif] leading-none not-italic relative shrink-0 text-[16px] text-center text-white" style={{ letterSpacing: "0.042em" }}>
            {label}
          </p>
        </div>
      </div>
...
// These are the 32×32 version of the Orderly icon used in mobile nav bar and drawer.
const MOBILE_SVG = {
...
/** Orderly "O" icon at configurable size — used in mobile nav bar and drawer top bar. */
...
/**
 * Full-screen navigation overlay — used for both mobile (< 600 px) and tablet
 * (600–1023 px) via the `deviceLayout` prop.
 *
 * Features:
 *   - Accordion sections for Builders / Ecosystem / Traders
 *   - Direct links for Docs and Campaigns (with CampaignIcon flame)
 *   - Bottom CTAs: "Launch Now" (white pill)
 *   - Framer Motion enter / exit animation (fade + slide down)
 *
 * Usage:
 *   <AnimatePresence>
...
export function MobileNavDrawer({
  ...
  return (
    ...
    >
      ...
      >
        <Link href="/" onClick={onClose} style={{ display: "block" }}>
      ...
      <nav className="flex-1 overflow-y-auto">
        {/* Accordion sections: Builders / Ecosystem / Traders */}
        ...
        {/* Campaigns — direct link with flame icon */}
        <div style={{ borderBottom: rowDivider }}>
          <a
            href={CAMPAIGNS_LINK.href}
            target="_blank"
...
/**
 * Fixed top bar for mobile (< 600 px).
 * Shows the Orderly "O" icon on the left and a hamburger button on the right.
 *
 * Mount it OUTSIDE any scaled/transformed wrapper so `position: fixed` works.
 * Pair with `MobileNavDrawer` inside `<AnimatePresence>`.
 *
 * Props:
 *   onMenuClick   — called when hamburger button is pressed
 *   bannerHeight  — offset from top if an announcement banner is shown (default 0)
 *
 * Usage:
 *   const [navOpen, setNavOpen] = useState(false);
 *   <MobileFixedNav onMenuClick={() => setNavOpen(true)} bannerHeight={bannerH} />
 *   <AnimatePresence>
 *     {navOpen && <MobileNavDrawer onClose={() => setNavOpen(false)} />}
 *   </AnimatePresence>
 */
export function MobileFixedNav({
  ...
  return (
    ...
    >
      <Link
        href="/"
        style={{ display: "block", width: 32, height: 32, position: "relative" }}
      >
...
/**
 * Sticky top nav bar for tablet (600–1023 px).
 * Shows the full Orderly logo (scaled 80%) on the left, and on the right:
 * a "Launch Now" pill CTA + hamburger button.
 *
 * Pair with `MobileNavDrawer` (deviceLayout="tablet") inside `<AnimatePresence>`.
 *
 * Props:
 *   onMenuClick   — called when hamburger button is pressed
 *   bannerOffset  — sticky `top` offset if an announcement banner is shown (default 0)
 *
 * Usage:
 *   const [navOpen, setNavOpen] = useState(false);
 *   <TabletNav onMenuClick={() => setNavOpen(true)} bannerOffset={bannerH} />
 *   <AnimatePresence>
 *     {navOpen && (
 *       <MobileNavDrawer
 *         key="tablet-nav"
